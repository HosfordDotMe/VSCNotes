...
function note(relativePath: string = path.join('path', 'note'), { fileLastModifiedAt, tags }: NoteMetadata = {}): Note {
    return {
...
suite('Notes', () => {
    ...
    suite('Working with directories', () => {
        ...
        test('Returns directories in the root of the store, one directory with multiple notes', () => {
            const notes = new Notes([note(path.join('dir', 'note1.md')), note(path.join('dir', 'note2.md'))])

            expect(notes.directories()).to.deep.equal(['dir'])
        })

        test('Returns directories in the root of the store, multiple directories', () => {
            const notes = new Notes([note(path.join('dir1', 'note.md')), note(path.join('dir2', 'note.md'))])
        ...
        test('Returns subdirectories of a directory, one subdirectory', () => {
            const notes = new Notes([note(path.join('dir', 'note.md')), note(path.join('dir', 'subdir', 'note.md'))])
    ...
    suite('Getting notes in a directory', () => {
        ...
        test('Returns notes in a subdirectory of the store, one note', () => {
            ...
            expect(notes.inDirectory('subdir').get()).to.deep.equal([note(path.join('subdir', 'note.md'))])
        })

        test('Returns notes in a subdirectory of the store, multiple notes', () => {
            const notes = new Notes([note(path.join('subdir', 'note1.md')), note(path.join('subdir', 'note2.md'))])
    ...
    suite('Filtering by tags', () => {
        ...
        test('Returns notes tagged with a specific tag, multiple tags, none matching', () => {
            const notes = new Notes([note('note1.md', { tags: ['tag1'] }), note('note2.md', { tags: ['tag2'] })])
        ...
        test('Returns notes tagged with a specific tag, multiple tags, one matching', () => {
            ...
            expect(notes.withTag('tag1').get()).to.deep.equal([note('note1.md', { tags: ['tag1'] })])
        })

        test('Returns notes tagged with a specific tag, multiple tags, multiple matching', () => {
            const notes = new Notes([note('note1.md', { tags: ['tag'] }), note('note2.md', { tags: ['tag'] })])
