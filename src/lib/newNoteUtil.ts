...
function replaceTokens(stringWithTokens: string, title: string, tokens: Token[]): string {
    ...
    tokens.forEach(token => {
        ...
        switch (token.type) {
            case 'datetime':
                replacement = moment().format(token.format)
                break
            case 'title':
                replacement = title
                break
...
async function createTemplate(template: Template, title: string, tokens: Token[]): Promise<void> {
    ...
    const inserted: boolean = await vscode.window.activeTextEditor.insertSnippet(new vscode.SnippetString(templateBody))
    if (inserted) {
...
export default async function createNote(noteFolder: string, template?: Template): Promise<void> {
    const config = vscode.workspace.getConfiguration('vscnotes')
    ...
    if (noteTitles.length > 0) {
        fileNameTemplate =
            (await vscode.window.showQuickPick([fileNameTemplate, ...noteTitles], {
    ...
    fileName = noteTitleConvertToLowerCase ? fileName.toLocaleLowerCase() : fileName
