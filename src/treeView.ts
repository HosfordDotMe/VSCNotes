...
export default class TreeDataProvider implements vscode.TreeDataProvider<TreeItem> {
    ...
    #onDidChangeTreeData: vscode.EventEmitter<TreeItem | undefined> = new vscode.EventEmitter<TreeItem | undefined>()
    ...
    constructor() {
        ...
        const ignorePatterns = (config.get<string[]>('ignorePatterns') ?? []).map(pattern => new RegExp(pattern))
