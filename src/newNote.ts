...
export async function newNote(workspaceFolder?: string): Promise<void> {
    const config: vscode.WorkspaceConfiguration = vscode.workspace.getConfiguration('vscnotes')
    ...
    const item: TemplateQuickPickItem | undefined = await vscode.window.showQuickPick(quickPickItems, {
...
export async function newNoteInWorkspace(): Promise<void> {
    ...
    const item: WorkspaceFolderQuickPickItem | undefined = await vscode.window.showQuickPick(
